...
def out_pdf(idx, dt, cli, ender, apr, items, tot):
    try:
        ...
        print(f"Erro ao gerar PDF: {err}")  # Log de erro
        return None
...
if aba == "🏠 Painel Principal":
    ...
    # Padroniza colunas para checagem segura
    ...
    if not df_c.empty:
        ...
        st.info("Nenhum cliente listado. Se você configurou a planilha agora, verifique o ID ou clique em 'Sincronizar'.")
    ...
    if not df_cl.empty and nome_col:
        ...
        st.warning("Adicione dados na planilha primeiro ou verifique a conexão.")
